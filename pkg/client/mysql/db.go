...
func (b *SelectQuery) Count() (count uint32, err error) {
	...
	b.SelectBuilder.LimitCount = -1
	b.SelectBuilder.OffsetCount = -1
	b.SelectBuilder.Column = []interface{}{"COUNT(*)"}
	b.SelectBuilder.Order = []dbr.Builder{}

	if isDistinct {
		b.SelectBuilder.Column = []interface{}{fmt.Sprintf("COUNT(DISTINCT %s)", getColumns(column))}
		b.SelectBuilder.IsDistinct = false
	}
